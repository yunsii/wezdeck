...
internal sealed class RequestRouter
{
    ...
    public string HandleRequestJson(string requestJson, string transport, Action<string> reportFailure)
    {
        ...
        var requestCategory = "wezdeck_runtime";
        string traceId = string.Empty;

        try
        {
            var request = JsonSerializer.Deserialize<RuntimeRequest>(requestJson, new JsonSerializerOptions
            {
            ...
            return JsonSerializer.Serialize(RuntimeResponse.Success(traceId, outcome));
        }
        catch (Exception ex)
        {
            ...
            return JsonSerializer.Serialize(RuntimeResponse.Failure(traceId, requestDomain, requestAction, "request_failed", ex.Message));
        }
    }
