...
internal static class RuntimeCliBootstrapLog
{
    ...
    public static int ExitWithError(string? message, string stage, long elapsedMs, string? exceptionType = null, string? hresult = null)
    {
        try
        {
            var text = string.IsNullOrWhiteSpace(message) ? "wezdeck-runtime-cli failed" : message;
            ...
            File.AppendAllText(
                Path.Combine(logDir, "wezdeck-runtime-cli-bootstrap.log"),
