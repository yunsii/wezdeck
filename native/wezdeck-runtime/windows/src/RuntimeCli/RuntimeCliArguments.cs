...
internal sealed record RuntimeCliRequestArgs(
    string PipeEndpoint,
    string PayloadBase64,
    int TimeoutMs);

internal static class RuntimeCliArguments
{
    public static bool TryParseRequest(string[] args, out RuntimeCliRequestArgs? request, out string? error)
    {
        ...
        {
            error = "usage: wezdeck-runtime-cli.exe request --pipe <endpoint> --payload-base64 <payload> [--timeout-ms 5000]";
            request = null;
            return false;
        }

        request = new RuntimeCliRequestArgs(pipeEndpoint, payloadBase64, timeoutMs);
        return true;
    }
}
