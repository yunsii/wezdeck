...
function Get-ProjectPaths {
  ...
  $projectRoot = Join-Path (Split-Path -Parent $RuntimeRoot) '.wezterm-native\wezdeck-runtime\windows\src'
  $nativeRoot = Split-Path -Parent $projectRoot
  return @{
    Manager = Join-Path $projectRoot 'Runtime\WezDeck.Runtime.csproj'
    Client = Join-Path $projectRoot 'RuntimeCli\WezDeck.RuntimeCli.csproj'
  ...
  switch ($RequestedSource) {
    ...
    'release' {
      if ($null -eq $ReleaseManifest -or $ReleaseManifest.enabled -ne $true) {
        throw 'release fallback is not configured in native/wezdeck-runtime/windows/release-manifest.json'
      }
      return 'release'
    }
    default {
      ...
      throw 'dotnet SDK is not installed on Windows and no wezdeck-runtime release package is configured'
    }
  }
}
...
function Test-ReleaseInstallCurrent {
  ...
  $managerPath = Join-Path $ResolvedInstallRoot 'wezdeck-runtime.exe'
  $clientPath = Join-Path $ResolvedInstallRoot 'wezdeck-runtime-cli.exe'
...
function Get-ReleasePreloadRoot {
  return Join-Path $env:LOCALAPPDATA 'wezterm-runtime\artifacts/wezdeck-runtime'
}
...
function Resolve-ReleaseDownloadUrl {
  ...
  $overrideUrl = [string]$env:WEZDECK_RUNTIME_RELEASE_URL
  ...
  $baseUrl = [string]$env:WEZDECK_RUNTIME_RELEASE_BASE_URL
...
function Ensure-ReleaseArchive {
  ...
  $explicitArchivePath = [string]$env:WEZDECK_RUNTIME_RELEASE_ARCHIVE
...
function Resolve-ExpandedPackageRoot {
  ...
  $managerPath = Join-Path $ExpandedDir 'wezdeck-runtime.exe'
  $clientPath = Join-Path $ExpandedDir 'wezdeck-runtime-cli.exe'
  ...
  if ($children.Count -eq 1) {
    ...
    $nestedManagerPath = Join-Path $nestedRoot 'wezdeck-runtime.exe'
    $nestedClientPath = Join-Path $nestedRoot 'wezdeck-runtime-cli.exe'
    if ((Test-Path -LiteralPath $nestedManagerPath) -and (Test-Path -LiteralPath $nestedClientPath)) {
      return $nestedRoot
    }
  }

  throw "expanded helper package missing wezdeck-runtime.exe or wezdeck-runtime-cli.exe: $ExpandedDir"
}
...
function Install-FromLocalBuild {
  ...
  $tempOutput = Join-Path ([System.IO.Path]::GetTempPath()) ("wezterm-wezdeck-runtime-" + [guid]::NewGuid().ToString('N'))
  $tempClientOutput = Join-Path ([System.IO.Path]::GetTempPath()) ("wezterm-wezdeck-runtime-cli-" + [guid]::NewGuid().ToString('N'))
  ...
  try {
    ...
    $installedManagerPath = Join-Path $ResolvedInstallRoot 'wezdeck-runtime.exe'
    $stoppedProcessIds = @(Stop-InstalledRuntimeProcesses -BinaryPath $installedManagerPath)
    Write-StructuredLog -Level 'info' -Category 'wezdeck_runtime' -Message 'stopping existing helper manager for install' -Fields @{
      trigger = $Trigger
...
function Install-FromRelease {
  ...
  try {
    ...
    $installedManagerPath = Join-Path $ResolvedInstallRoot 'wezdeck-runtime.exe'
    $stoppedProcessIds = @(Stop-InstalledRuntimeProcesses -BinaryPath $installedManagerPath)
    Write-StructuredLog -Level 'info' -Category 'wezdeck_runtime' -Message 'stopping existing helper manager for release install' -Fields @{
      trigger = $Trigger
...
Write-StructuredLog -Level 'info' -Category 'wezdeck_runtime' -Message 'starting windows helper install' -Fields @{
  trigger = $Trigger
...
Write-Output ("[helper-install] installed_binary=" + (Join-Path $InstallRoot 'wezdeck-runtime.exe'))
Write-Output ("[helper-install] installed_client=" + (Join-Path $InstallRoot 'wezdeck-runtime-cli.exe'))
Write-StructuredLog -Level 'info' -Category 'wezdeck_runtime' -Message 'installed windows helper manager build' -Fields @{
  trigger = $Trigger
  restart_reason = if ($resolvedInstallSource -eq 'local') { 'sync_install' } else { 'release_install' }
  runtime_dir = $RuntimeDir
  install_root = $InstallRoot
  install_source = $resolvedInstallSource
  installed_binary = (Join-Path $InstallRoot 'wezdeck-runtime.exe')
  installed_client = (Join-Path $InstallRoot 'wezdeck-runtime-cli.exe')
}
Write-Output (Join-Path $InstallRoot 'wezdeck-runtime.exe')
