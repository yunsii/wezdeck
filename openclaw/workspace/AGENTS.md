...
5. **错误闭环** — 检测→诊断→合理自愈→验证→汇报。裸错误 / 未解码的 `🛠️ Exec failed` / `Process: … failed` 不合规（须同轮人话：失败/原因/处置/影响）。细节：`skills/error-closed-loop/SKILL.md`。
...
12. **一任务一树一写者** — `claw-*` worktree；`dirname(primary)/.worktrees/<repo>/`；不与 live CLI/ACP 双写；不写 primary。
