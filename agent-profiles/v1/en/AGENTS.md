...
- Human-run handoff: if a **human** must run a script, **mandatory** load skill `human-run` → tell user `x` → `ensure-env` + `propose.sh --cwd …` (default `--wait`, like a normal shell task) → continue on completion; never multi-line paste-to-terminal; never wrap agent self-exec. Full rules in [reporting.md](./reporting.md) `Human-run handoff`.
