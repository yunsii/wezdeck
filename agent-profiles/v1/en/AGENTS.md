...
- Dependency / upstream / third-party / internal-service fault triage; also out-of-branch faults (CI/platform flake) → confirm same-repo mainline / owning tip has a fix before designing (prefer web+tracker for public deps; org tools for internal; OSS issues/discussions + source-indexed / browser evidence) → [dependency-triage.md](./dependency-triage.md)
...
- Dependency triage: when the fault likely sits in a dependency — or evidence says it is outside the current branch's ownership — run the out-of-branch gate first (fresh `origin/HEAD` / owning tip before designing a parallel fix); then classify public-external / vendor / internal; public and OSS → prioritize current web + Issues/Discussions/changelog (plus source-indexed upstream analysis and live browser when needed); internal → org-local logs/deploy/service tools, not public-web rabbit holes. Full playbook in [dependency-triage.md](./dependency-triage.md).
...
- Adversarial review: multi-role find+refute (not solo monologue); **agent** runs repo/OpenClaw skill + `skills/adversarial-review/run.sh` — humans only state intent; disclose writer/form/backends. Full rules in [validation.md](./validation.md) `Adversarial review`.
...
- Human-run handoff: if a **human** must run a script, **mandatory** load skill `human-run` → tell user `x` → `ensure-env` + `propose.sh --cwd …` (default `--wait`) → agent continues; handoff body = non-blocking kickoff only (no CI waits in `x`); never multi-line paste-to-terminal; never wrap agent self-exec. Full rules in [reporting.md](./reporting.md) `Human-run handoff`.
