...
- Human-run handoff: if a **human** must run a script, **mandatory** load skill `human-run` → `ensure-env.sh` → `propose.sh --cwd …` → tell user `x`; never multi-line paste-to-terminal; never wrap agent self-exec. Full rules in [reporting.md](./reporting.md) `Human-run handoff`.
