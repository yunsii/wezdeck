...
# $WEZDECK_REPO — other-repo agents often have WEZDECK_REPO on an older checkout.
set -euo pipefail
...
# Skill dir = …/skills/human-run (follow ~/.agents/skills links)
source_file="$(readlink -f "${BASH_SOURCE[0]}")"
skill_dir="$(cd "$(dirname "$source_file")/.." && pwd -P)"
# Sibling under repo: skills/human-run → scripts/runtime/cli/wd-run
colocated_raw="$skill_dir/../../scripts/runtime/cli/wd-run"
colocated=""
if [[ -e "$colocated_raw" ]]; then
  colocated="$(cd "$(dirname "$colocated_raw")" && pwd)/$(basename "$colocated_raw")"
fi

# Colocated BEFORE WEZDECK_REPO / possibly-stale agent-tools: the linked skill
...
[[ -n "${WEZDECK_REPO:-}" ]] && candidates+=("$WEZDECK_REPO/scripts/runtime/cli/wd-run")
