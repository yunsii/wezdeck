...
describe('glyph tint on an active counter', function()
  ...
  it('paints the glyph with _glyph and the label with _fg', function()
    with_live_entries(function()
      assert_eq(color_of('▲', PALETTE), '#b45309', 'waiting glyph tint')
      assert_eq(color_of('1 waiting', PALETTE), '#78350f', 'waiting label color')
      assert_eq(color_of('✓', PALETTE), '#15803d', 'done glyph tint')
      assert_eq(color_of('1 done', PALETTE), '#14532d', 'done label color')
      assert_eq(color_of('●', PALETTE), '#0369a1', 'running glyph tint')
      assert_eq(color_of('1 running', PALETTE), '#0c4a6e', 'running label color')
    end)
  end)
  ...
  it('falls back to _fg when a palette omits the _glyph key', function()
    with_live_entries(function()
      ...
      assert_eq(color_of('▲', legacy), '#78350f', 'waiting glyph did not fall back')
      assert_eq(color_of('✓', legacy), '#14532d', 'done glyph did not fall back')
      assert_eq(color_of('●', legacy), '#0c4a6e', 'running glyph did not fall back')
    end)
  end)
end)
