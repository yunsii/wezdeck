...
attention_picker_emit_frame() {
  ...
  for (( top_index = start_index; top_index <= end_index; top_index += 1 )); do
    ...
    # All badges land at 6 cells (glyph 1 + space 1 + 4-char text).
    # See cmd_attention.go's coloredBadge for why this set is monochrome
    # 1-cell text code points rather than color emoji.
    case "$status" in
