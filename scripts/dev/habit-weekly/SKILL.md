...
description: >
  ...
  CDP verify→iterate, and hotkey intensity (Alt+l etc.). Use when the user says
...
数据收集器是 `scripts/dev/habit-report.sh`（插件化 Claude / Grok / Codex）。本 skill 负责：**定窗口 → 拉 JSON → 按固定模板渲染周报 →（可选）落盘**。

## When to load / run

| 用户意图 | 你做 |
| --- | --- |
| 开发习惯周报 / 习惯周报 / habit weekly | `run.sh`（默认本周迄今） |
| 上周习惯 / last week | `run.sh --week last` |
| 自定义区间 | `run.sh --since YYYY-MM-DD --until YYYY-MM-DD` |
| 只要原始 JSON | `run.sh --json-only` |
| 落盘归档 | `run.sh --write`（再 `--stdout` 可同时打印正文） |
...
1. **定窗口**（用户没说则默认本周迄今 Mon→today）：
   ...
   - 点名日期 → `--since` / `--until`
2. **跑收集 + 渲染**（优先 `--write` 落盘，便于复盘）：
   ```bash
   "$R" --week this --write
   ```
   或：
   ```bash
   "$R" --since 2026-09-11 --until 2026-09-17 --write --stdout
   ```
3. **读** `references/REPORT_STYLE.md`，核对渲染稿是否需补一句人话结论（数字已被模板写好；只在环路归纳与分工上可加 1–2 句，**禁止改数字**）。
4. **汇报**：把 markdown 正文（或落盘路径 + 摘要）用简体中文交给用户。披露数据窗口与 `max_running` 口径（pane+TTL）。
...
1. 一句话结论  
2. Agent 并发（pane+TTL；raw_sid 仅诊断）  
3. Skill / CLI / MCP + 分 provider  
4. CDP 验证→迭代（有则写）  
5. 热键调度（Alt+l 为主）  
6. 工作环路归纳  
7. 数据边界 + 复现命令  
...
- Collector: `scripts/dev/habit-report.sh` + `scripts/dev/habit_report/providers/`  
