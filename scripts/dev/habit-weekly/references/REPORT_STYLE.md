...
7. **代码变更量**（可选插件 `git_churn`；工作机 auto；排除仓/生成目录见 `~/.config/habit-weekly/git-churn.json`）  
8. **Skill / CLI / MCP**（合并 Top + 分 provider 一句分工）  
9. **CDP 验证→迭代**（有数据才写）  
10. **热键调度**（Alt+l 为主；忙日/轻量日对照）  
11. **工作环路**（短流程图或 5 步列表）  
12. **数据边界**（固定注意点，可压缩；注明 WakaTime ≠ WezDeck 口径）  
13. **复现命令**  
14. **本次提取的数据**（结尾必有：表列本 run 实际采到的信号与有/无/未启用）  
## 文风
