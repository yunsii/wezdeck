...
DEFAULT_MANIFEST_PATH="$REPO_ROOT/native/wezdeck-runtime/windows/release-manifest.json"

usage() {
  cat <<'EOF'
Usage:
  scripts/dev/update-wezdeck-runtime-release-manifest.sh --tag TAG --sha256 HEX [options]

Options:
  --tag TAG              Release tag, for example wezdeck-runtime-v2026.04.19.1
  --sha256 HEX           SHA-256 for the published release zip
  --repo OWNER/REPO      GitHub repository slug used to derive the download URL
  --asset-name NAME      Asset filename. Defaults to wezdeck-windows-runtime-<version>-win-x64.zip
  --url URL              Full download URL. Overrides --repo derived URL
  --output PATH          Output path. Defaults to native/wezdeck-runtime/windows/release-manifest.json
...
if [[ -z "$asset_name" ]]; then
  asset_version="${tag#wezdeck-runtime-}"
  asset_name="wezdeck-windows-runtime-${asset_version}-win-x64.zip"
fi
