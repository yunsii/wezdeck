...
#   2. If $WEZTERM_REPO/cli lacks wd-run/x/lib, symlink from that tree
...
SKILL_DIR="$TOOL_HOME"
# scripts/dev/human-run → wezdeck root
...
REPO="${WEZTERM_REPO:-$HOME/github/wezterm-config}"
...
if [[ -d "$REPO/scripts/runtime" ]] || [[ -d "$REPO" ]]; then
  ...
  log "WEZTERM_REPO=$REPO missing; skipping PATH tree install (agent-tools will point at skill tree)"
fi
