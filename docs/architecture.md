...
- Managed Codex resume uses `scripts/runtime/codex-resume-takeover.sh` before `codex resume --last`. It treats writer locks as disposable markers: a lock with no holder is removed directly; a lock with a holder is terminated, then removed, and the takeover is logged under `primary_pane`. The rollout JSONL under `$CODEX_HOME/sessions/` is retained, so a newly opened managed pane can take ownership of the resume instead of remaining blocked behind an abandoned writer lock.
