...
- Every agent-CLI launch path — workspace first-open, `Alt+g` on-demand window, `refresh-current-window`, tab-overflow cold-spawn — terminates at `scripts/runtime/agent-launcher.sh <profile>`. The launcher is the single env-loading site (it sources `scripts/runtime/runtime-env-lib.sh::runtime_env_load_managed`) so secrets reach the agent regardless of whether the chain traverses a zsh rc file. Adding a new entry path means routing it through `agent-launcher.sh`; do not invoke `claude` / `codex` directly from a `tmux new-window` / `respawn-pane` call site. The `${WEZTERM_REPO}` placeholder in `config/worktree-task.env` is expanded by `scripts/runtime/worktree/lib/resume-command.sh`, `scripts/runtime/tab-overflow-cold-spawn.sh`, and `wezterm-x/lua/config/managed_cli.lua::parse_managed_cli_env` — keep all three in lockstep.
