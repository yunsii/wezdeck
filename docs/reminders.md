...
- **tmux binary mismatch after upgrade**. If the running tmux server is a newer user build (WezDeck: `~/.local/bin/tmux` ≥ 3.7; historically `/usr/local/bin/tmux` 3.6a) but cron's default `PATH=/usr/bin:/bin` finds apt `/usr/bin/tmux` 3.4, the version-skewed client cannot talk to the server: `tmux list-clients` prints `server exited unexpectedly` to stderr (discarded by cron) and `tmux-popup-active.sh` silently no-ops. Fix in `wezterm-x/local/crontab` by putting the **user tmux dir first**, e.g. `PATH=/home/<you>/.local/bin:/usr/local/bin:/usr/bin:/bin` (cron honors env declarations for all subsequent CMDs), then reinstall with `crontab wezterm-x/local/crontab`. Policy and install: [`tmux-install.md`](./tmux-install.md).
