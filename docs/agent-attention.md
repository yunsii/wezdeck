...
Clearance paths for elicitation waiting: watcher (dialog gone), `Stop → done`, next `UserPromptSubmit → running`, `Alt+/` Ctrl+X clear (or clear-all), TTL. Not PostToolUse.
...
- *Fail-open layers (not the same as product gates).* Agent CLIs (Claude / Grok) **ignore non-zero hook exits** (`user_prompt_submit hook … failed, ignored`) so a broken emitter never blocks a turn — that is the outer safety net. Inside `emit.sh`, `set -u` still applies: every optional sticky field (`prompt_summary`, …) must be initialized, and state/OSC steps soft-fail with `|| true`, otherwise a nounset abort skips the badge write for that event even though the agent continues. Non-zero exits leave `level="error" message="hook aborted"` in `runtime.log` (EXIT trap) so operators need not paste the CLI toast — see [`diagnostics.md`](./diagnostics.md) Hook / wrapper self-check. Product gates (waiting whitelist, tmux race, `AGENT_ATTENTION_SKIP`, no jumpable pane) are intentional `exit 0` skips, not crashes. Regression: `b411eab` referenced `prompt_summary` before assignment → UserPromptSubmit aborted under `set -u`.
...
- *Exit paths.* An entry leaves `entries` through one of these paths. Paths marked **[archived]** copy the departing entry into `recent[]` (see *Recent archive* below) before removing it from `entries`; paths without the marker drop the entry without archiving (rare — currently only same-session in-place transitions, which technically aren't exits at all):
  ...
  2b. **`Alt+/` Ctrl+X per-entry clear** — archives+forgets the selected live `running` / `waiting` / `done` row only (`attention-jump.sh --forget <session_id>`). Popup stays open so multiple stuck rows can be cleared without reopening. Distinct from ◆ SB Ctrl+X soft-stop (same chord, different row kind). **[archived]**
...
- *Recent archive.* Every exit path marked **[archived]** above pushes the departing entry onto `attention.json`'s top-level `recent[]` array via the shared `archive_into_recent` jq helper in `attention-state-lib.sh`. Each tombstone preserves the entry's tmux/wezterm coordinates plus sticky display fields (`last_user_prompt`, `agent_name`, `tmux_window_name`, `git_branch`), `last_status` (the status held at archive time), `last_reason`, `live_ts` (the entry's `ts` at archive time), and `archived_ts`. Disk dedup key is `(tmux_socket, tmux_session, tmux_pane)` so repeated archives of the same pane (e.g. multiple `/clear`s in pane A, overflow rotation back-and-forth) collapse into the newest tombstone instead of accumulating one row per `session_id`, while sibling panes in the same tmux session each keep their own slot. An earlier (socket, session)-only key let one pane's archive silently overwrite another pane's history; pane ids `%N` are server-internal monotonic identifiers stable enough to be the third dimension. The picker is tmux-only (`Alt+/` short-circuits outside tmux), so any rows the hook writes from non-tmux contexts aren't jumpable — the recent-row keep-filter in `tmux-attention-menu.sh` drops them from display by requiring `tmux_socket` and `tmux_window` to be non-empty *and* the **window** to still be in the per-socket alive set (pane recycle after split/kill must not hide a still-jumpable worktree; Enter does `select-window` then lands on a live pane in that window). Newer `archived_ts` wins on collision; the array is capped at 50 entries (`ATTENTION_RECENT_CAP`) and TTL'd at 7 days (`ATTENTION_RECENT_TTL_MS`), enforced on every archive call so the array stays bounded even when the picker never opens. The `Alt+/` picker shows recent entries under a `○ RCNT` band after the live waiting/done/running rows: Lua `compute_picker_data` dedups **display** by `(tmux_session, tmux_window)` (one worktree window → one row — collapsing by session alone hid sibling worktrees under a single project), sorts by activity (`live_ts` / `archived_ts`), and caps at `PICKER_RECENT_CAP` (20). Row body prefers `agent_name · last_user_prompt` and never surfaces canned Stop copy (`task done` / `end_turn`) as the only text; label leaf prefers `tmux_window_name` over `git_branch`. Selecting a row drops the file trigger described in *Picker dispatch* below, with `kind="recent"` and a re-resolved WezTerm pane id (the picker queries `tmux show-environment WEZTERM_PANE` on the target session before writing the trigger so the stored id, which is whatever was live at archive time, is replaced with the currently-live one — WezTerm reassigns pane ids on restart while tmux survives, and trusting the stored id would silently land the activate on a phantom). The Lua-side dispatch attempts the in-process mux activate; if the WezTerm pane is gone, `activated=false` shows up in the trigger jump log and the user sees the tmux side jump but the GUI not follow. As a separate safety net the picker also runs a cheap one-shot `tmux list-windows -a` per recent socket at popup-open time so rows whose worktree window is gone are filtered out before the user can pick them, and the legacy `attention-jump.sh --recent --session <id> --archived-ts <ms>` path stays available as a fallback (it probes **window** existence, falls back to a live pane in that window when the archived pane id is gone, and removes the row from `recent[]` only when the window itself is dead).
...
- `Alt+j` / `Alt+k` / `Alt+l` / `Alt+Shift+l` are Lua `action_callback`s (not tmux forwarders). They call `attention.pick_next` on the current state (scoped to `waiting`, `done`, and `running` respectively; `Alt+Shift+l` is `running` with `opts.reverse`), then `attention.activate_in_gui` performs `SwitchToWorkspace` when needed, plus mux-level `tab:activate()` and `pane:activate()` so the target becomes visible even across WezTerm OS windows and workspaces. The tmux `select-window`/`select-pane` runs in the background via `scripts/runtime/attention-jump.sh --direct --tmux-socket … --tmux-window … [--tmux-pane …]` spawned through `wsl.exe` from Lua — the entry already carries the coordinates, so the fast path skips the state re-read, `jq` invocations, and the redundant `wezterm.exe cli activate-pane`. Entries without tmux coordinates (legacy / partial) fall back to `--session <id>`, which runs the full resolution path.
  - *`pick_next` filters by tmux pane, not wezterm pane, and walks the pool round-robin.* In this repo's split-pane layout a single wezterm pane commonly hosts a whole tmux session whose split panes have independent focus. Focused-slot detection prefers live `M.is_entry_focused` (wezterm pane hosts `tmux_session` AND tmux-focus file matches `entry.tmux_pane`); when that misses, it falls back to the per-kind cursor written by `M.note_jump` after each successful Alt+j/k/l pick. The cursor is what keeps rapid `Alt+l` cycling alive when the tmux-focus file lags the jump (cross-FS write + hook lag, or a no-op `select-*` that never fires `after-select-pane`): without it `focused_idx` stayed nil and every press re-picked `pool[1]`, which `activate_in_gui` resolved to the already-focused wezterm pane — the key looked dead. A sibling tmux pane in the same wezterm pane stays a valid jump target. Legacy / non-tmux entries (no `tmux_session`) keep the historical wezterm-pane comparison. Order is spatial: current workspace first (left→right by live tab index / pane left-top), then other workspaces by name with the same L→R within each; `ts` is only a tiebreak. Once the cursor index is known, the next press returns the following slot and wraps — so multiple `●` in one workspace are swept before any cross-workspace hop. **Sole-entry nil only on live focus:** when the pool has one entry, `pick_next` returns nil only if `is_entry_focused` says the user is already on it. A `note_jump` cursor alone must not suppress — otherwise Alt+l to the sole `●`, manual workspace/tab switch away, then Alt+l again logs `alt-l jump running empty` while the counter still shows 1. Empty picks log `alt-l jump running empty` (and the j/k equivalents) so a "key did nothing" report can tell silent-nil from a missing binding. Coverage: `tests/lua-units/test_pick_next.lua` walks the multi-tmux-pane topology, the 3-entry round-robin case, the `note_jump` focus-lag case, the sole-entry-after-leave case, the same-workspace-before-cross-workspace spatial case, plus cross-wezterm-pane and legacy-fallback regressions.
  - *`--direct` status refresh is no-op-only.* After `select-window` / `select-pane`, `attention-jump.sh --direct` only runs `tmux-status-refresh.sh --force --no-debounce --refresh-client` when both selects were already active (hooks would not fire — the parked-on-target case from `3d26108`). A real window/pane change leaves refresh to `session-window-changed` / `window-pane-changed` (debounced). Forcing `--no-debounce` on every Alt+l previously stampeded ~300ms git/status probes and made rapid cycling feel frozen even when the Lua side had already returned.
- After `activate_in_gui` succeeds on a `done` entry (via either `Alt+k` or `Alt+/`), the Lua side additionally spawns a background `attention-jump.sh --forget <session_id> --delay 3 --only-if-ts <ts>` as a safety-net cleanup. In practice the focus-based auto-ack described under *Rendering* fires first (on the next `update-status` tick after the jump lands) with the zero-delay path and the optimistic in-memory hide, so the counter drops almost immediately — the three-second delayed forget only matters if focus-ack never ran (for example, the user jumped away before the next tick). `waiting` entries are *not* auto-acked by focus: jumping with `Alt+j` lands on the prompt but does not clear the counter — the user must actually answer (the badge clears later, when the approved tool finishes and `PostToolUse → resolved` fires; if the user denies, the badge clears on the next `Stop` / `UserPromptSubmit` / TTL) or press **`Ctrl+X` in `Alt+/`** to archive that one row. `running` entries are never forgotten by a jump (`Alt+l` / Enter): they stay informational until a real status transition — **except** the explicit `Alt+/` Ctrl+X clear (and clear-all / TTL / pane-evict), which is the recovery path when an agent died without firing `Stop` and left a stuck `●`.
- `Alt+/` is a forwarded shortcut: the WezTerm `attention.overlay` handler first calls `attention.write_live_snapshot(constants.attention.live_panes_file)` so a fresh `pane_id → {workspace, tab_index, tab_title}` JSON is on disk at `state/agent-attention/live-panes.json` (atomic temp + rename, payload includes `ts` epoch ms), then forwards `\x1b/` to the tmux pane. The same writer is also re-fired on every `update-status` tick, throttled to one rewrite per `LIVE_SNAPSHOT_INTERVAL_MS` (1s by default — tracked via `last_live_snapshot_ms`, which both writers share so an Alt+/ press resets the throttle). The tick refresh is what makes the read side trivial: `tmux-attention-menu.sh` just `jq -c '.panes // {}'`s the file with no freshness gate and no trace adoption, because the producer side maintains the snapshot continuously rather than coupling it to the press. The earlier press-time-only design was racy across the WSL/Windows mount — `os.rename` was not always visible to the bash reader before the forwarded `\x1b/` reached tmux, and the menu would then read a multi-minute-old file, fail a 5-second freshness gate, and render every label as `?/?/`. Tmux's `M-/` binding runs `scripts/runtime/tmux-attention-menu.sh`, which opens `tmux display-popup -E scripts/runtime/tmux-attention-picker.sh`. The picker reads state.json *and* the snapshot file directly — no `wezterm.exe cli list` round-trip from the popup pty, no dependency on `WEZTERM_UNIX_SOCKET` env propagating through tmux into the popup. After a WezTerm restart the snapshot may briefly carry pane-id keys from the previous instance; the first `update-status` tick after `wezterm.lua` reloads (≤250 ms) overwrites them, and a missing key still degrades to `?` per slot anyway. Rows are shaped: `<workspace>/<repo>/<tmux_window>_<tmux_pane>/<branch>  <marker> <reason>  (<age>[, no pane])`. Identity is the tmux session, so the tab segment is the **session repo** (parsed from `tmux_session = wezterm_<ws>_<repo>_<hex>`), not the wezterm tab index — a session keeps its label across spawn-cap eviction, workspace close+reopen, and overflow rotation, even though those events change the wezterm tab id. Slot separators are `/`; within the tmux slot the glue is `_` so no terminal-convention glyphs (`#`, `@`, `:`, `%`) leak into the label. Unknown components render as `?`; when all four are unknown the prefix is omitted entirely. Combined with the per-pane dedup at the write layer (`attention_state_upsert` drops other entries sharing the same `(tmux_socket, tmux_session, tmux_pane)`), this guarantees one row per active tmux pane.
  ...
  - **Status filter chip.** `Tab` cycles an orthogonal status filter `all → waiting → done → running → sb → all`, shown as a colored chip in the title (`[▲ waiting]` / `[✓ done]` / `[● running]` / `[◆ SB watch]`). Status filter and substring filter AND together — substring narrows whatever passes the status cycle. **`Ctrl+X`** clears the selected row without jumping (popup stays open): on live attention (`●` / `▲` / `✓`) it runs `attention-jump.sh --forget <session_id>` (archives into `recent[]`, nudges the counter); on **`sb`** rows (session-bridge Ctrl+K w watches injected by `scripts/runtime/session-bridge-watch-picker-rows.sh`) it soft-stops that watch (`active=false`, job file kept for audit — does **not** touch agent-attention entries). Plain `x` is always a search character so you can filter for “x”. **Enter** still jumps (attention or SB pane). The `clear all · N entries` sentinel is hidden whenever any filter is active so it cannot be confused with a real entry; it returns when both filters are at their default. When the combined filter excludes everything, the picker shows `No matches — Esc clears search, Tab cycles status, Backspace edits.` in place of the rows; the popup stays open so the user can recover without retyping.
...
- *Sticky / overflow inconsistency alerts.* When a sticky tab still *names* a session (or sticky `visible_set` still claims it) but the jump must project into `…`, or collision immediately fights that project back to browse, Lua emits `level="warn"` lines whose `message` starts with `inconsistent:`. Grep recipe and field table: [`tab-visibility.md`](./tab-visibility.md) *Consistency alerts*.
...
- *Delegated / non-interactive agents (invariant).* Anything that is **not** your interactive TUI session must **not** write the human attention badge. That includes OpenClaw C3 ACP (Claude-ACP / Codex-ACP), `cross-repo-delegate` workers, adversarial-review / brainstorm headless, and CI `claude -p` / `codex exec`. Layers:
  - **Spawner** sets `AGENT_ATTENTION_SKIP=1` (aliases: `WEZTERM_ATTENTION_SKIP`, `DELEGATE_HEADLESS`, `OPENCLAW_ACP`) and clears `WEZTERM_PANE` / `TMUX` / `TMUX_PANE`. Claude headless also passes `--settings '{"disableAllHooks":true}'` (still loads project `AGENTS.md`, unlike `--bare`).
  - **OpenClaw ACP wrappers** (`~/.openclaw/acpx/*-wrapper.mjs`) are patched by `openclaw/scripts/patch-acpx-attention-skip.sh` (re-run after `@openclaw/acpx` regenerates them).
  - **Write side** (`emit.sh`): skip when any skip env is truthy, or when `OPENCLAW_ACPX_LEASE_ID` is set (`hook skipped: AGENT_ATTENTION_SKIP` / `OPENCLAW_ACPX_LEASE_ID`).
