...
- *Exit paths.* An entry leaves `state.json` through one of these paths:
  1. **30-minute TTL** at the next prune (write-time or periodic).
  2. **`Alt+/` clear-all sentinel** — wipes every entry.
  3. **Same-session overwrite** — a fresh `Stop` or non-waiting `Notification` on the same `session_id` (a `waiting` upsert against an existing `waiting` is a no-op; see *Waiting is sticky* above).
  4. **Same-pane eviction by a different session** — an upsert from a *different* `session_id` that lands on the same `(tmux_socket, tmux_pane)`. A pane hosts at most one active attention entry, so restarting an agent in place evicts the prior one instead of double-counting.
  5. **Jump-to-done forget** — a successful `Alt+.` / `Alt+/` jump to a `done` entry immediately spawns `attention-jump.sh --forget <session_id> --only-if-ts <ts>`. The `--only-if-ts` guard keeps a fresher `done` that reused the same `session_id` during the ~50 ms subprocess window from being wiped.
  6. **Periodic background prune** — see *Periodic cleanup* below.
  7. **Focus-based auto-ack** — `done` only (`waiting` is intentionally excluded so a glance does not silently swallow a pending prompt). Uses the same zero-delay `--forget` with `--only-if-ts` guard. See *Rendering* below.
  8. **Test-only `cleared`** — `scripts/dev/test-agent-attention.sh cleared` or the `--clear-all` sentinel; never wired to a Claude hook.
  9. **`pane-evict` on `/clear`** — `SessionStart` with `matcher: "clear"` wipes every entry on the current `(tmux_socket, tmux_pane)` except the new `session_id`. See *SessionStart pane eviction* above.
