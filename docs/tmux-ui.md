...
- **Grok Build light TUI background.** Stock GrokDay paints `bg_base` as cool `#eeeeee`, which clashes with this repo's cream pane (`#eae9e1` inactive / `#f1f0e9` active). Grok has no public custom-theme API, so `scripts/dev/patch-grok-theme-wezdeck.sh` patches the single ratatui Rgb tag in `~/.grok/bin/grok` from `#eeeeee` → `#eae9e1` (override with `WEZDECK_GROK_BG=f1f0e9` if you prefer the active colorscheme base). Re-run after every Grok self-update. Pin light mode with `auto_light_theme = "grokday"` in `~/.grok/config.toml`.
