...
- An entry can exit state.json through seven paths: (1) a `UserPromptSubmit` on the same session, (2) the 30-minute TTL at the next prune, (3) the `Alt+/` clear-all sentinel, (4) a fresh `Notification`/`Stop` with the same session_id that overwrites it, (5) an upsert from a *different* session_id that lands on the same `(tmux_socket, tmux_pane)` — a pane hosts at most one active attention entry, so restarting an agent in place evicts the prior one instead of double-counting — (6) a successful `Alt+.` / `Alt+/` jump to a `done` entry, which schedules a delayed `--forget` against `attention-jump.sh` that removes the entry after a 3-second grace window (the delayed forget carries `--only-if-ts <ts>` so a fresher `done` that reused the same `session_id` during the grace window is not wiped), or (7) the periodic background prune described in *Periodic cleanup* below.
