...
- `scripts/claude-hooks/emit-agent-status.sh` is the sole writer. Claude Code hooks (configured in user-level `~/.claude/settings.json`) map events to statuses: `Stop` → `done`, `UserPromptSubmit` → `cleared` (which removes the entry), `PostToolUse` → `resolved` (conditional remove: only drops the entry when its current status is `waiting`), and `Notification` branches on `notification_type` — `permission_prompt` / `elicitation_dialog` → `waiting`, `idle_prompt` → `done` (since it fires on post-Stop idle and would otherwise re-raise waiting), `auth_success` is ignored. This keeps `waiting` meaning "Claude needs user action" rather than "Claude is idle after a turn".
- *Waiting is sticky.* Inside `attention_state_upsert`, a `waiting` upsert on a session whose current status is already `waiting` is a no-op — `ts`, `reason`, and tmux coordinates all stay at their first-waiting values. Only a non-waiting upsert (normally `done`) transitions the entry out. This prevents the counter from oscillating when Claude fires multiple permission prompts inside a single turn, and it keeps the 30-minute TTL clock anchored to when the session first blocked for input instead of to the most recent prompt. `cleared` (via `UserPromptSubmit`) still removes the entry unconditionally.
- *Waiting auto-resolves on tool completion.* The `PostToolUse` hook fires `emit-agent-status.sh resolved`, which calls `attention_state_clear_if_waiting` — a conditional remove that deletes the entry if and only if its status is `waiting`, otherwise it is a no-op. A completed tool is treated as evidence that the user resolved the permission prompt (allowed), so the `⚠` counter clears as soon as the tool finishes instead of waiting for `Stop`. `done` entries are left alone so focus-ack still owns that clearance path. Denied permission (tool never runs) leaves the `waiting` entry in place; it clears when the next `Stop` / `UserPromptSubmit` / TTL fires. The shell-side fast path peeks at the state file without the flock and returns a no-op status code when there is nothing to remove, so emit-agent-status.sh can skip the OSC tick and avoid hammering wezterm on every auto-allowed tool call.
...
- An entry can exit state.json through nine paths: (1) a `UserPromptSubmit` on the same session, (2) the 30-minute TTL at the next prune, (3) the `Alt+/` clear-all sentinel, (4) a fresh `Stop` or non-waiting `Notification` on the same session_id that overwrites it (a `waiting` upsert against an existing `waiting` is a no-op — see *Waiting is sticky* below), (5) an upsert from a *different* session_id that lands on the same `(tmux_socket, tmux_pane)` — a pane hosts at most one active attention entry, so restarting an agent in place evicts the prior one instead of double-counting — (6) a successful `Alt+.` / `Alt+/` jump to a `done` entry, which schedules a delayed `--forget` against `attention-jump.sh` that removes the entry after a 3-second grace window (the delayed forget carries `--only-if-ts <ts>` so a fresher `done` that reused the same `session_id` during the grace window is not wiped), (7) the periodic background prune described in *Periodic cleanup* below, (8) the focus-based auto-ack described in *Rendering* below, which uses the same delayed `--forget` with `--only-if-ts` guard so a fresh `done` inside the 3-second window survives, or (9) a `PostToolUse` → `resolved` transition described in *Waiting auto-resolves on tool completion* below, which conditionally removes only `waiting` entries.
