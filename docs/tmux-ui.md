...
- tmux refresh (respawn) is command-palette-owned instead of WezTerm-shortcut-owned. The lighter **layout fix** is on `Ctrl+k r` / palette `Session: Fix layout`: `scripts/runtime/tmux-fix-layout.sh` runs `refresh-client -S`, rebalances the current window with `even-horizontal`, clamps multi-line `status` to ≤3 lines, and forces a status recompute. Typical triggers are RDP attach/detach and Windows DPI / zoom changes that leave splits off-centre or inflate the status strip; it does not respawn panes. Palette `Session: Refresh current window` still respawns the focused pane and now runs the same heal first.
