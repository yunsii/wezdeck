...
- **Grok Build light TUI background / focus flash.** Stock GrokDay paints `bg_base` as cool `#eeeeee`. This repo's opaque preset uses `#eae9e1` (inactive pane) and `#f1f0e9` (active pane / WezTerm `palette.background`). Grok enables focus tracking and often full-repaints on focus-in; if `bg_base` ≠ the *active* cream, that first paint frame flashes. Grok has no public custom-theme API, so `scripts/dev/patch-grok-theme-wezdeck.sh` patches the single ratatui Rgb tag in `~/.grok/bin/grok` to **`#f1f0e9`** (override with `WEZDECK_GROK_BG=…`). Re-run after every Grok self-update. Pin light mode with `auto_light_theme = "grokday"` in `~/.grok/config.toml`.
