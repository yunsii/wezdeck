...
- The `Alt+g` worktree picker carries an agent-status column: worktree label + `[branch]` is padded to a common width (measured over every row, not just the visible window, so scrolling does not shift the column) and the trailing cell shows `▲ 2m` / `● 12s` / `✓ 30s` for a live agent entry, dimmed `(new)` for a worktree with no tmux window, and nothing at all when the worktree has a window but no pending agent state — same meaning as a tab with no badge (the dimmed `last ✓ 3m` archived form was tried and dropped; see [`agent-attention.md`](./agent-attention.md)). Glyph vocabulary matches `Alt+/` on purpose — both pickers answer "who needs attention", so they should read the same; the tab strip is the one surface that drops the glyph for a color block (see [`agent-attention.md`](./agent-attention.md)). The dim runs restore with the background-preserving SGR so the selected-row bar stays continuous.
