...
- Live TUI popups are a poor fit for mouse text selection (plain drag is unbound; `Shift+drag` enters copy-mode against a redrawing TUI; `Super+drag` is awkward on Windows). Pickers that expose a copyable payload therefore ship a keyboard copy that does **not** rely on OSC/DCS pass-through: tmux `display-popup -E` does not forward those sequences to the parent WezTerm client (same constraint as the attention picker's file event-bus). `Ctrl+y` in the links picker copies the URL via `Set-Clipboard`; in the `Alt+g` worktree picker `Ctrl+y` copies the focused path, `Ctrl+b` copies the branch name via `agent-clipboard.sh` (Windows WezDeck Runtime), and `Ctrl+d` reclaims the focused linked worktree after an Enter/y confirm (same dirty/delivery gates as `Ctrl+k g r`; see [`keybindings.md`](./keybindings.md)). The worktree picker paints `path · branch` on a detail line under the title so both copy payloads are visible before copy (plus ` · <reason>` when the focused worktree has agent-attention state).
...
| Cool-grey canvas / no active↔inactive cream (two Grok panes look identical) | Stock GrokDay `bg_base` is opaque `#eeeeee`, covering tmux `window-style` `#eae9e1` / `window-active-style` `#f1f0e9` | **Standing:** focus-filter launch ensure / `--install` re-applies `scripts/dev/patch-grok-theme-wezdeck.sh` on `grok.real` (default `Color::Reset`). Opt out: `WEZDECK_GROK_THEME_PATCH=0`. Pin `auto_light_theme = "grokday"`. |
