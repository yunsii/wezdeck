...
- `Ctrl+k` `p`: toggle the focused agent pane between a bare CLI session and a Happy-wrapped (phone-sync) session, resuming the same conversation. Bare→Happy registers the session for phone control in the Happy app; Happy→bare returns to a plain local session. `agent-happy-toggle.sh` reads the agent flavour and wrap-state from the pane's descendant process tree, then `respawn-pane -k`s that one pane through `agent-launcher.sh <flavor> [--happy]`. No-op on non-agent panes; claude only for now (codex promotion is gated pending verification). Simple version — assumes one conversation per cwd; see [`mobile-access.md`](mobile-access.md).
