...
- `Alt+v`: in any tmux-backed pane, forward to tmux so the active tmux window resolves the live worktree first and, in `hybrid-wsl`, uses the Windows native helper request path; outside tmux, WezTerm opens the current worktree root in VS Code directly and still falls back to pane handling when it only sees the WSL host path. Existing VS Code windows are reused by `distro + folder`; set `WEZTERM_VSCODE_PROFILE` in `wezterm-x/local/shared.env` to choose the VS Code profile used by Alt+v (default `ai-dev`), and set `WEZTERM_VSCODE_MAX_WINDOWS` to cap launches for new folders. When the cap is reached, the helper reuses the least recently used helper-tracked VS Code window with `--reuse-window --folder-uri` instead of launching another one. There is no hotkey for opening a *specific* file — that path is agent-driven: `scripts/runtime/open-file-in-vscode.sh <file>` focuses/opens the file's repo window (same `distro + folder` reuse) and reveals the file in it via `--file-uri`, so an agent can auto-open a proposal it just generated for review
- `Alt+g`: in any tmux-backed pane, open a centered tmux popup worktree picker for the current repo family; selecting an unopened worktree creates its tmux window on demand. Each row carries that worktree's agent state in a trailing column — `🚨 2m` / `🔄 12s` / `✅ 30s` for a live agent-attention entry, a dimmed `last ✅ 3m` for one that already left the live set (focus-ack / TTL / `/clear` archived it), and the original `(new)` hint for a worktree with no tmux window yet. The focused row's detail line appends the entry's reason (`path · branch · needs your permission to use Bash`), which is the one thing the row itself has no room for. Row order stays the worktree-list order so the `1-9,0,a-z` accelerators never move. Where the state comes from: [`agent-attention.md`](./agent-attention.md). Inside the picker: `Enter` / `1-9,0,a-z` open the focused (or accelerator) worktree; `Ctrl+y` copies the focused worktree's absolute path and `Ctrl+b` copies its branch name to the Windows clipboard via `scripts/runtime/agent-clipboard.sh` (host helper IPC — not OSC 52; tmux popups do not forward OSC to WezTerm) and keep the picker open (detail line shows `path · branch [· reason]`; flash is `copied path …` / `copied branch …` on success, `no branch to copy` when detached/empty, or `copy failed` if the helper errors). Mouse text selection inside this live TUI is unreliable: plain drag is unbound by design, `Shift+drag` enters tmux copy-mode and fights the redraw, and `Super+drag` is awkward on Windows where Super is Win. `Esc` / `Ctrl+c` / a second `Alt+g` close
