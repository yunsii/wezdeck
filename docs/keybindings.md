...
- `Alt+/`: open a centered tmux popup picker listing every pending task. Rows are `<workspace>/<tab_index>_<tab_title>/<tmux_window>_<tmux_pane>/<branch>  <marker> <reason>  (<age>[, no pane])`, where marker is `⚠` waiting / `⟳` running / `✓` done. The popup has a command-palette-style always-on `Search:` input on row 2 (dim `Type to filter (Tab cycles status)…` placeholder when empty); typing any printable ASCII filters by case-insensitive substring against the row body (workspace / tab / branch / reason all match). `Backspace` edits, `Ctrl+U` clears the query in one keystroke, `Esc` clears a non-empty query first and closes only on the second press. `Up`/`Down` move, `Enter` jumps. `Tab` cycles an orthogonal status filter `all → waiting → done → running → all` shown as a chip in the title. Press `Alt+/` or `Ctrl+C` again to close from any state (true toggle). Last row is a destructive `——  clear all · N entries  ——` sentinel (hidden when any filter is active) for recovering stale entries (WezTerm restart, agents killed without hooks).
