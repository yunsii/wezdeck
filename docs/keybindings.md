...
- `Alt+g`: in any tmux-backed pane, open a centered tmux popup worktree picker for the current repo family; selecting an unopened worktree creates its tmux window on demand. Inside the picker: `Enter` / `1-9,0,a-z` open the focused (or accelerator) worktree; `Ctrl+y` copies the focused worktree's absolute path and `Ctrl+b` copies its branch name to the Windows clipboard via `scripts/runtime/agent-clipboard.sh` (host helper IPC — not OSC 52; tmux popups do not forward OSC to WezTerm) and keep the picker open (detail line shows `path · branch`; flash is `copied path …` / `copied branch …` on success, `no branch to copy` when detached/empty, or `copy failed` if the helper errors). Mouse text selection inside this live TUI is unreliable: plain drag is unbound by design, `Shift+drag` enters tmux copy-mode and fights the redraw, and `Super+drag` is awkward on Windows where Super is Win. `Esc` / `Ctrl+c` / a second `Alt+g` close
