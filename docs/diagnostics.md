...
3. **cron-driven reaping has not yet been observed succeeding on its own** (2026-07-29). Every run before the `XDG_RUNTIME_DIR` fix was a silent no-op, and the only `reaped expired uxc daemon sessions` entries so far came from manual invocations. Closes by finding one with a `trace_id` that does not match a hand-run: `grep 'reaped expired uxc daemon sessions' ~/.local/state/wezterm-runtime/logs/runtime.log`. If a week passes with sessions expiring and no such entry, re-check the cron line and the socket path rather than assuming it works.
4. **Next.js dev servers reach ~6 Gi of swap on their own** (2026-07-29). Independent of the MCP work: one `next dev` process was found holding 5.95 Gi of swap with `VmHWM` 6.8 Gi, was cleaned up, and a freshly started one reached 6.1 Gi again within hours. Nothing in this repo manages those processes; restarting them periodically is currently the only mitigation. Worth a decision on whether that belongs in `agent-cleanup.sh`'s scope or stays manual.
