...
- `bin/helper-install-state.json` — written by the PowerShell installer at the end of every successful install. Its **mtime** is sync-runtime's "last successful helper install" marker; `find -newer` on `native/wezdeck-runtime/windows/src/**` and the `release-manifest.json` against this file decides whether `dotnet publish` runs again.
- `state/helper/state.env` — written by the running wezdeck-runtime every ~250ms. `ready=1` + filesystem mtime within ~10s of now is sync-runtime's "helper alive" signal that lets `helper-ensure` skip the PowerShell round-trip. CRLF line endings (PowerShell-written) — readers must strip `\r` before string-comparing values.
- To force-stop a helper during canary cleanup or recovery, run `scripts/dev/stop-wezdeck-runtime.sh --canary` (live helper: no flag; all helper managers: `--all`). The next WezTerm/helper ensure recreates the daemon.
...
- WezDeck Runtime reuse diagnostics emit explicit decision fields such as `decision_path`, `registry_hit`, `matched_process_count`, `matched_process_ids`, and `matched_window_found`.
...
- **Primary (pluginized):**
  ...
  - Optional collectors (`habit_report/plugins/`): **auto-detect**. `--plugins auto|off|all|rime,git_churn`.
    - **rime**: enables when counter lua / `rime-commits.jsonl` exists; `plugins.rime` (+ alias `rime_commits`); join `host.foreground` **and** `wezterm-pane-focus.jsonl` (from `tmux-focus-emit.sh` on pane focus change) into `wezterm.agent.*` / `wezterm.shell`. Install: [`scripts/dev/rime-commit-counter/`](../scripts/dev/rime-commit-counter/).
    - **git_churn**: enables on `workflow.device_profile = 'work'` or `~/.config/habit-weekly/git-churn.json`; scans `roots` with `git log --numstat`; configurable `exclude_repos` / `exclude_repo_globs` / `exclude_path_globs` (example: [`git_churn.example.json`](../scripts/dev/habit_report/git_churn.example.json)). No commit messages in payload.
...
`coco-weekly-report` (business delivery / Feishu). **Not** linked by
`scripts/dev/link-platform-skills.sh` (stays under this repo; needs
co-located `habit-report.sh`).
...
WezDeck Runtime samples the OS foreground process name on each heartbeat and logs
...
- The Windows host smoke test validates both text and image clipboard IPC, including the tracked [`copy-test.png`](../scripts/dev/windows-runtime-host/cases/fixtures/copy-test.png) fixture.
...
- If the whole distro disappears — or WezTerm "silently quits" with no Lua error — do **not** start in WezTerm config. First-triage is [`development-environment-troubleshooting.md#the-whole-wsl-distro-disappears`](./development-environment-troubleshooting.md#the-whole-wsl-distro-disappears): `dmesg` continuity (distro restart vs VM reboot), then `%LOCALAPPDATA%\Temp\wsl-crashes\kernel-panic-*.txt` (MCE / Fatal machine check — reference 2026-09-23), then the previous journal for `oom-kill` / memory peak. Memory-class hardening: [`guest-oom.md`](./guest-oom.md).
...
- **Hook / wrapper self-check (no need to paste CLI "hook failed, ignored").** Agent CLI hooks and the Grok focus-filter wrapper write warn/error into `~/.local/state/wezterm-runtime/logs/runtime.log` (`attention` / `primary_pane`). When a hook aborts under `set -u` or ensure/theme heal fails, look here first:
  ...
  Managed agent startup rows (`message="agent resume boot"`) include
  `agent`, `mode`, and `permission_profile`; `permission_profile=base` means
  the agent's own default configuration, while a named value such as
  `full-access` records the resolved permission intent. The companion
  `permission_resolution=profile` field confirms that the tracked overlay was
  ensured before the provider adapter built its CLI argv.
...
- In `hybrid-wsl`, WezTerm prewarms the WezDeck Runtime during GUI startup, then still falls back to on-demand ensure when the helper later goes stale or bootstrap state is missing.
- To reproduce the release fallback on a machine that already has Windows `dotnet`, run sync with `WEZDECK_RUNTIME_INSTALL_SOURCE=release` and inspect `helper-install-state.json` plus the `[helper-install]` terminal lines for `installed_source`, `release_version`, and the installed binary paths.
- If GitHub downloads are too slow, place the zip at `%LOCALAPPDATA%\wezterm-runtime\artifacts/wezdeck-runtime\<version>\<assetName>` or set `WEZDECK_RUNTIME_RELEASE_ARCHIVE`, then rerun sync and confirm `release_archive_source=preload_versioned|preload_flat|explicit_archive`.
...
11. **The VS Code Z-order LRU fix ships only to `install_source=local` installs** (2026-08-08). It was verified by hand-dropping a locally built `wezdeck-runtime.dll` into `%LOCALAPPDATA%\wezterm-runtime\bin\`, previous binary kept beside it as `wezdeck-runtime.dll.bak-preZorderLru`. Two loose ends, and they point in opposite directions. **This machine is fine**: `helper-install-state.json` says `source: local`, so `install-wezdeck-runtime.ps1` rebuilds from the working tree and a reinstall carries the fix — but the bytes currently running came from a manual `cp`, not from that script, so `bin/` is not in a state the installer produced and the `.bak-` file is litter until someone reinstalls. **`install_source=release` installs are not fine**: `native/wezdeck-runtime/windows/release-manifest.json` still pins the pre-fix build, so they keep the old behaviour — the reused window locks onto whichever one the runtime recorded first — with no visible signal that the two disagree. Closes by cutting a WezDeck Runtime release per [`wezdeck-runtime-release.md`](./wezdeck-runtime-release.md), updating the manifest, and deleting the `.bak-preZorderLru` file. Until then, `decision_path="max_windows_reuse_zorder_lru_window"` in this machine's `helper.log` says nothing about what a release install does.
...
15. **WSL Fatal machine check (MCE) recurrence / host WHEA echo unverified** (2026-09-23). Guest dump proved `Kernel panic - not syncing: Fatal machine check` (CPU 14, bank `b200000080060001`) at 13:49:28; triage path recorded in [`development-environment-troubleshooting.md#fatal-machine-check-mce--reference-2026-09-23`](./development-environment-troubleshooting.md#fatal-machine-check-mce--reference-2026-09-23). What is *not* closed: whether Windows **WHEA-Logger** / Hyper-V-Worker logged a matching host event, and whether this was a one-off thermal/power glitch or a standing hardware path. Closes by pairing the next `kernel-panic-*.txt` MCE (or confirming none for a quiet month) with Event Viewer timestamps; escalate to memory diagnostic / XMP / undervolt review only on recurrence.
16. ~~**Cross-agent Auto / Full access profile parity was only implemented for Codex**~~ → **closed 2026-09-25**. `MANAGED_AGENT_PERMISSION_PROFILE` now maps Claude `full-access` to `--permission-mode bypassPermissions`, Grok `full-access` to `--always-approve`, and Codex to its native overlay. `auto` leaves each provider on its base permission configuration. Workspace/repo `permission_profile` overrides propagate through both Lua first-open and shell resume paths; launch logs record the resolved profile and resolution source.
