...
- **Primary (pluginized):**
  ...
  - Optional collectors (`habit_report/plugins/`): **auto-detect**. `--plugins auto|off|all|rime,git_churn`.
    - **rime**: enables when counter lua / `rime-commits.jsonl` exists; `plugins.rime` (+ alias `rime_commits`); join `host.foreground`. Install: [`scripts/dev/rime-commit-counter/`](../scripts/dev/rime-commit-counter/).
