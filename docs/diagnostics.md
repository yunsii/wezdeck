...
4. **Concurrent sessions share one MCP process, and therefore share its page state** (2026-07-29). This is a genuine regression against resident MCP, which gave every agent session its own process. The daemon keys sessions on `stdio:{endpoint}:{auth_fingerprint}` — the caller's identity is not part of it — so every agent invoking `chrome-devtools-mcp-cli` lands on the same child, and the selected page plus snapshot `uid`s live in that child. Two sessions interleaving within the 600 s window can silently cross wires: A selects page 2, B selects page 5, A's next `take_snapshot` returns page 5 without erroring. Wrong data, no failure signal.
   Two mitigations exist, both with costs. `--experimentalPageIdRouting` removes the global selected-page entirely — measured: it turns `take_snapshot` into `required: ['pageId']` and `click` into `required: ['pageId', 'uid']` — but that **breaks every example in upstream's skill** (`click uid=3_0` starts failing on a missing `pageId`), and upstream documents no such mode. Alternatively, a distinct endpoint string yields a distinct `session_key` and thus a separate process, so per-context links would isolate at the cost of giving back the single-instance win. Neither is applied yet: the collision needs two agents driving the browser inside the same 10-minute window, which is plausible here but not routine. Escalate to `--experimentalPageIdRouting` the first time a snapshot is observed returning the wrong page — do not wait for a second occurrence, since the failure is silent.
