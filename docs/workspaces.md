...
- After the respawn, `tmux-reset/window.sh` tags the pane with `@wezterm_pane_role=agent-cli:<base>` (or clears the option for non-agent respawns) so the `Ctrl+n` / `Ctrl+P` bindings can recognize agent panes through the `sh -c '<resume> || exec <fresh>'` wrapper's `pane_current_command=sh`/`node` boot transient. The shared `@agent_pane_match` predicate (defined at the top of `tmux.conf`) layers this **intent tag** with the leaf-name fallback (`claude*` / `codex*`) and a shell-veto, so manually-launched agent panes still work without a tag and the binding falls through cleanly once a shell takes over the pane. See the `Ctrl+n` entry in `keybindings.md` for the full predicate.
