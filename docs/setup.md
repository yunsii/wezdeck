...
- **Keys**:
  ...
  - `agent_clipboard` — absolute path to `scripts/runtime/agent-clipboard.sh`. Bash script; callable only from WSL. Writes text or an image file to the Windows clipboard via wezdeck-runtime named-pipe IPC.
  - `open_file_in_vscode` — absolute path to `scripts/runtime/open-file-in-vscode.sh`. Bash script; callable only from WSL. Takes one file path (relative to the caller's cwd or absolute) and reveals it in VS Code, focusing/opening the file's repo window via the same wezdeck-runtime pipeline as `Alt+v`. Use it to auto-open a file you generated for the user to review.
  - `wd_run` — absolute path to `scripts/runtime/cli/wd-run`. Human-handoff propose/peek/run CLI (`x` is the short human path). Only for commands a human must run; see [`agent-run.md`](./agent-run.md).
- **Sample**:
  ...
  wd_run=/home/yuns/github/wezterm-config/scripts/runtime/cli/wd-run
  ```
...
The badge reflects what the Windows wezdeck-runtime reads from the foreground WezTerm window, not WezTerm's internal `use_ime` flag. When another Windows application is foreground, the helper holds the last WezTerm sample instead of borrowing that application's IME mode:
...
This section applies **only to maintainers** who cut releases or develop the native components (Windows wezdeck-runtime / Go popup picker). Regular contributors and end users can skip it — the prerequisites above are sufficient for daily use, since native components ship as prebuilt binaries via [`wezdeck-runtime-release.md`](./wezdeck-runtime-release.md) and [`picker-release.md`](./picker-release.md). End users without a Go or .NET toolchain still get the fast Go picker through the release-manifest fetcher in `native/picker/build.sh` (auto mode).

- `gh` (GitHub CLI) **required** for the release flow in [`wezdeck-runtime-release.md`](./wezdeck-runtime-release.md). It pushes tags, watches the release workflow, merges the auto-generated manifest-update PR, and toggles the repo Actions permission described below. Install with `sudo apt install gh` on Ubuntu/Debian, `brew install gh` on macOS, or follow <https://cli.github.com/>. After install, authenticate once with `gh auth login` and verify with `gh auth status`.
...
- `dotnet 8.0+` SDK on Windows **required** to build `native/wezdeck-runtime/windows/...` locally and to verify the local-build install path with `WEZDECK_RUNTIME_INSTALL_SOURCE=local` ([`wezdeck-runtime-release.md#forcing-the-release-path-locally`](./wezdeck-runtime-release.md#forcing-the-release-path-locally)). Not required for cutting releases — the GitHub Actions runner installs its own SDK via `actions/setup-dotnet@v4`. Install from <https://dotnet.microsoft.com/download/dotnet/8.0>, or with `winget install Microsoft.DotNet.SDK.8`. Verify with `dotnet --list-sdks` from a PowerShell prompt.
