...
- `tmux 3.7+` must be available in the runtime environment that will host managed project tabs. Required because the repo's `tmux.conf` advertises DEC mode 2026 (synchronized output), where tmux 3.6 introduced the 1-second flush timeout needed to avoid stuck sync windows, and because copy-mode auto-refresh uses tmux 3.7's `refresh-from-pane`. Ubuntu 24.04 LTS still ships 3.4, so build 3.7+ from source if your distro lags. Background, verification recipe, and the IME-flicker symptom that originally drove the tmux floor: [`ime-flicker-and-sync-output.md`](./ime-flicker-and-sync-output.md).
