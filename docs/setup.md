...
- `go 1.21+` **required** in the WSL/Linux side to build the static `scripts/runtime/picker/bin/picker` ELF that powers the three high-frequency tmux popups: `Alt+/` (attention), `Alt+g` (worktree), and `Ctrl+Shift+P` (command palette). `wezterm-runtime-sync`'s `build-picker` step (`scripts/runtime/picker/build.sh`, invoked from `skills/wezterm-runtime-sync/scripts/sync-runtime.sh`) auto-discovers `go` in `PATH` → `~/.local/go/bin/go` → `/usr/local/go/bin/go` and silently skips with a one-line note when none are found; the popups then fall back to bash pickers (`tmux-attention-picker.sh`, `tmux-worktree-picker.sh`, `tmux-command-picker.sh`), which work but cold-start at ~30-80ms vs ~2-5ms for the Go binary (~10×, per `docs/performance.md`). Only direct dep is `golang.org/x/term`. Install with `sudo apt install golang-go` on Ubuntu 24.04+ (ships ≥ 1.22), or download from <https://go.dev/dl/> into `~/.local/go`. After install, run `wezterm-runtime-sync` once and confirm `scripts/runtime/picker/bin/picker` exists and the sync trace logs `step=build-picker status=completed`. Until the picker release pipeline lands, end users who want the speedup today can install Go themselves; afterwards prebuilt binaries arrive via the manifest pipeline like `host-helper`.
