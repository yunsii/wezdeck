...
| worktree subsystem | 5,390 | 32 | **Mostly stays.** `[worktrees]` + `herdr worktree` cover create/open/remove against a plain `<dir>/<repo>/<branch-slug>` layout. The dev/task/hotfix semantics, repo-family reuse, task-prompt injection, reclaim, and the `${WEZDECK_REPO}` expansion are ours. |
