...
Most sync steps short-circuit when their inputs haven't changed since the last successful run, so a "nothing changed" sync settles around **~700ms** (vs ~16s pre-optimization, ~60s before that — see [`performance.md#sync-runtime-hot-path`](./performance.md#sync-runtime-hot-path) for the data trail). Each gate has a force-bypass env var when you need it to run regardless:
...
- During sync, [`scripts/dev/check-deps-updates.sh`](../scripts/dev/check-deps-updates.sh) is launched **detached** (`nohup ... &`) and **rate-limited to once per day** so the foreground sync never blocks on network calls. Output is written to `$WEZTERM_RUNTIME_STATE_DIR/logs/deps-check.log`; each sync prints either `[sync] deps-check running in background, log: <path>` (first run today) or `[sync] deps-check skipped (last run YYYY-MM-DD), log: <path>` (already ran). The script compares `wezterm` / `tmux` / `go` (installed vs upstream latest vs repo floor). For wezterm, hybrid-wsl tracks the **Windows nightly zip**: the row compares the installed build stamp (`YYYYMMDD-HHMMSS` from `wezterm --version`) with the `WezTerm-windows-nightly.zip` asset `updated_at` timestamp. The nightly release tag's `target_commitish` is logged as diagnostic context only because the rolling tag can point at stale commits and must not drive update status. The go row mirrors `native/picker/build.sh`'s discovery chain (PATH → `~/.local/go/bin/go` → `/usr/local/go/bin/go`) so a manual install under `~/.local/go` is recognized even when the sync shell didn't inherit the PATH addition. Offline upstreams degrade to `offline?`. Set `WEZTERM_SYNC_SKIP_DEPS_CHECK=1` to suppress the step entirely; set `WEZTERM_SYNC_FORCE_DEPS_CHECK=1` to bypass the daily gate and force a fresh run.
